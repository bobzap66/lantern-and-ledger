...
function hrefFor(file: QuartzComponentProps["allFiles"][number]) {
  if (!file.slug) return "#"
  const target = canonicalSlug(file.slug)
  const prefix = `${SEASON_OF_GHOSTS_ROOT}/`
...
export default (() => {
  ...
  const ArchiveReadingActions: QuartzComponent = (props: QuartzComponentProps) => {
    ...
    if (currentSlug !== SEASON_OF_GHOSTS_ROOT) {
    ...
    const sessions = allFiles
      .filter((file) => {
        ...
        return canonicalSlug(file.slug).startsWith(`${SEASON_OF_GHOSTS_ROOT}/`)
      })
    ...
    return (
      <section
        ...
        aria-label="Oral History reading shortcuts"
      >
        ...
        <div class="series-archive-actions__grid">
          <a href={hrefFor(first)} class="series-archive-action internal">
          ...
          <a href={hrefFor(current)} class="series-archive-action internal">
